...
Behavioral rules for Claude Code in the deskspace repository.
...
- **Axum 0.8 wildcard syntax:** `{*path}`, not `*path` — the bare form panics at runtime.
- **highlight.js has no browser-ready bundle.** The npm package ships CJS in `lib/` and ESM wrappers in `es/` that still import CJS. The `ui/` bundle must go through `bun build` before serving.
- **Frontend bundle must exist before `cargo run`:** run `cd ui && bun install && bun run build` before serving, or static assets will be missing.
...
**Prefer `cargo test -q`** — quiet mode only prints failures, reducing output noise and context usage.
